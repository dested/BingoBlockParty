...
namespace FarseerPhysics.Collision
{
    ...
    public class DynamicTreeBroadPhase : IBroadPhase
    {
        ...
        private void BufferMove(int proxyId)
        {
            if (_moveCount == _moveCapacity)
            {
                ...
                _moveCapacity *= 2;
                _moveBuffer = new int[_moveCapacity];
                ArrayUtils.Copy(oldBuffer, _moveBuffer, _moveCount);
            }
        ...
        private bool QueryCallback(int proxyId)
        {
            ...
            if (_pairCount == _pairCapacity)
            {
                ...
                _pairCapacity *= 2;
                _pairBuffer = new Pair[_pairCapacity];
                ArrayUtils.Copy(oldBuffer, _pairBuffer, _pairCount);
            }
        ...
        public void UpdatePairs(BroadphaseDelegate callback)
        {
            ...
            ArrayUtils.Sort(_pairBuffer, 0, _pairCount);
    ...
    { 
        public static void Copy<T>(T[] oldNodes, T[] nodes, int nodeCount)
        {
        }

        public static void Sort<T>(T[] pairBuffer, int i, int pairCount)
        {
        }
        
        public static void Sort<T>(List<T> internalList, IComparer<T> pointComparer)
        {
        }
        public static void Sort<T>(List<T> internalList, Comparer<T> pointComparer)
        {
        }
        public static IEnumerable<T2> Cast<T2>(this object val)
        {
            return (dynamic)val;
        }
        public static T2[] ToArray<T2>(this IEnumerable<T2> val)
        {
            var cc = 0;
